...
func main() {
	...
	* FIRST BLOCK
	...
	fmt.Println("Waiting 20 seconds to send the second block to the full node")
	...
	* SECOND BLOCK
	...
	if secondBlock == nil {
		fmt.Println("NewBlock returned nil")
	}
	...
	// Publish a message to ask for the blockchain
	fmt.Println("Sending a second block to full node")
	if err := blockAnnouncementTopic.Publish(ctx, secondBlockBytes); err != nil {
		fmt.Printf("Error publishing blockchain request : %s\n", err)
	}
