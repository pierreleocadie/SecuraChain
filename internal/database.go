...
type TransactionDatabase interface {
	SaveTransaction(key []byte, tx transaction.Transaction) error
	GetTransaction(key []byte) (transaction.Transaction, error)
	Close() error
}
...
func (pdb *PebbleTransactionDB) SaveTransaction(key []byte, tx transaction.Transaction) error {
	txBytes, err := json.Marshal(tx)
	if err != nil {
		return fmt.Errorf("erreur de sérialisation de la transaction: %v", err)
	}

	err = pdb.db.Set(key, txBytes, pebble.Sync)
	if err != nil {
...
func (pdb *PebbleTransactionDB) GetTransaction(key []byte) (transaction.Transaction, error) {
	txBytes, closer, err := pdb.db.Get(key)
	if err != nil {
		return nil, fmt.Errorf("erreur de récupération de la transaction: %v", err)
	}
	defer closer.Close()

	return transaction.DeserializeTransaction(txBytes, &transaction.AddFileTransactionFactory{})
}
