...
func GetFile(log *ipfsLog.ZapEventLogger, ctx context.Context, config *config.Config, ipfsAPI icore.CoreAPI, cidFile path.ImmutablePath) error {
	...
	if err != nil {
		log.Errorln("Could not get file with CID: %s", err)
	...
	if err != nil {
		log.Errorf("Could not write out the fetched CID: %v\n", err)
	...
	log.Debugln("Got file back from IPFS (IPFS path: %s) and wrote it to %s\n", cidFile.String(), downloadedFilePath)
...
func GetDirectoryWithPath(log *ipfsLog.ZapEventLogger, ctx context.Context, ipfsAPI icore.CoreAPI, directoryFile path.ImmutablePath, downloadPath string) error {
	local, err := os.Getwd()
	if err != nil {
		log.Errorln("Error getting current working directory")
		return err
	}
	...
	if err != nil {
		log.Errorln("Could not get directory with CID: %s", err)
	...
	go func() {
		...
		err = files.WriteTo(rootNodeFile, downloadedFilePath)
		if err != nil {
			log.Errorln("Could not write out the fetched CID: %v\n", err)
	...
	log.Debugln("Got directory back from IPFS (IPFS path: %s) and wrote it to %s\n", directoryFile.String(), downloadedFilePath)
